...
class CoachTest(TestCase):
    ...
    def test_randint_returning_max_value(self, mocked_re, mocked_bsp, mocked_get, mocked_randint):
        ...
        mocked_bsp.return_value = mock.MagicMock(
            find_all=mock.MagicMock(
                return_value=[
                    mock.MagicMock(get_text=mock.MagicMock(return_value='quote 1')),
                    mock.MagicMock(get_text=mock.MagicMock(return_value='quote 2'))
                ]
            )
        )
        ...
        coach(mock.MagicMock(), mock.MagicMock())
